> **✅ 2026-09-06 解決済み。** 真因は openssl 3.x と macOS の security の噛み合わせでした。
> `openssl pkcs12 -export` に **`-legacy`** を付ける1行の修正で通り、
> build 55 / 0.1.2 を App Store の審査に提出完了（WAITING_FOR_REVIEW）。
>
> **下の「有力な仮説」（証明書と秘密鍵のペアずれ）は外れていました。**
> 証明書も秘密鍵もパスワードも全て正しく、`ios-cert-diagnose.yml` で
> 公開鍵の一致を実測して確認しました。
> 問題は **P12 の暗号化方式** でした：openssl 3.x の既定で作ると
> macOS の `security` が読めず、「wrong password?」という誤導的なエラーになります。
>
> **教訓：こちらが何も変えていなくても、足元の環境（ランナーの openssl）が
> 動いて壊れることがある。** Secrets もワークフローも未変更なのに
> 7/4 から壊れたのはそのためです。
>
> 以下は当時の調査記録として残します。
